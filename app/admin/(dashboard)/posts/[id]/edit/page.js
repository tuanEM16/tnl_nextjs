...
export default function EditPostPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            toast.success('DỮ LIỆU ĐÃ ĐƯỢC TÁI CẤU HÌNH THÀNH CÔNG!');
            router.push('/admin/posts');
        } catch (error) {
            toast.error('CẬP NHẬT THẤT BẠI - KIỂM TRA LẠI SERVER');
        } finally {
            setLoading(false);
        }
    };

    if (fetching) return <div className="p-20 font-black italic animate-pulse uppercase tracking-[0.3em] text-center">Rebuilding Content Buffer...</div>;
    ...
    return (
        <div className="space-y-12 pb-20 font-archivo uppercase">
            ...
            <header className="flex justify-between items-end border-b-4 border-black pb-8">
                <div>
                    <p className="text-[10px] font-black tracking-[0.4em] text-gray-400 italic mb-2">Content Modification Unit</p>
                    <h1 className="text-7xl font-black tracking-tighter leading-none text-black">
                        CHỈNH SỬA<span className="text-orange-600">.</span>
                    </h1>
                </div>
                <button 
                    onClick={() => router.back()} 
                ...
                <div className="lg:col-span-2 space-y-12">
                    <section className="space-y-6">
                        <h2 className="flex items-center gap-3 text-xl font-black border-l-8 border-black pl-4 bg-gray-50 py-3 shadow-sm">
                            <MdArticle size={24}/> BIÊN TẬP NỘI DUNG
                        </h2>
                        
                        <div className="space-y-1">
                            <label className="text-[10px] font-black text-gray-400 italic">Tiêu đề bài viết</label>
                            <input
                                type="text"
                                name="title"
                                value={formData.title}
                                onChange={handleChange}
                                className="w-full border-2 border-black p-6 font-black text-2xl tracking-tighter outline-none focus:bg-black focus:text-white transition-all shadow-[6px_6px_0px_0px_rgba(0,0,0,0.1)]"
                                required
                            />
                        </div>

                        <div className="space-y-1">
                            <label className="text-[10px] font-black text-gray-400 italic">Mô tả tóm tắt (Sapo)</label>
                            <textarea
                                name="description"
                        ...
                        <div className="space-y-1">
                            <label className="text-[10px] font-black text-gray-400 italic">Nội dung chi tiết bài viết</label>
                            <textarea
                                name="content"
                    ...
                    <div className="sticky top-10 space-y-6">
                        <div className="bg-white border-2 border-black p-8 space-y-10 shadow-[10px_10px_0px_0px_rgba(0,0,0,1)]">
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="space-y-4">
                                    <div className="space-y-1">
                                        <label className="text-[10px] font-black text-gray-400 italic">Loại nội dung</label>
                                        <select
                                            name="post_type"
                                        ...
                                        >
                                            ...
                                            <option value="page">TRANG TĨNH (STATIC)</option>
                            ...
                            <div className="space-y-4">
                                <h3 className="text-sm font-black flex items-center gap-2 border-b-2 border-black pb-4 italic uppercase">
                                    <MdCloudUpload size={20}/> Ảnh đại diện
                                </h3>
                                <div className="relative border-2 border-dashed border-black p-4 text-center cursor-pointer hover:bg-gray-50 transition-all group">
                                    ...
                                    {preview ? (
                                        <div className="space-y-2">
                                            ...
                                            {!imageFile && oldImage && (
                                                <p className="text-[9px] font-black text-orange-600 italic tracking-widest">[ DỮ LIỆU ẢNH GỐC ]</p>
                                            )}
                                        </div>
                                    ) : (
                                        <div className="py-10">
                                            <MdCloudUpload size={32} className="mx-auto mb-2 text-gray-300 group-hover:text-black transition-colors" />
                                            <p className="text-[9px] font-black tracking-widest text-gray-400">THAY ĐỔI ẢNH</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            {/* TRẠNG THÁI HỆ THỐNG */}
                            <div className="space-y-1">
                                <label className="text-[10px] font-black text-gray-400 italic">Trạng thái xuất bản</label>
                                <select
                                    name="status"
                            ...
                            <button
                                type="submit"
                                disabled={loading}
                                className="w-full bg-black text-white py-8 text-xs font-black uppercase tracking-[0.4em] hover:bg-orange-600 transition-all flex items-center justify-center gap-3 shadow-[6px_6px_0_0_#ea580c] active:translate-x-[2px] active:translate-y-[2px] active:shadow-none"
                            >
                                <MdSave size={24} />
                                {loading ? 'UPDATING...' : 'CẬP NHẬT DỮ LIỆU →'}
                            </button>
                        </div>
                        ...
                        <div className="p-6 border-2 border-black border-dashed bg-gray-50/50">
                            <p className="text-[9px] font-black flex items-center gap-2 leading-relaxed">
                                ...
                                <span>THẬN TRỌNG: MỌI THAY ĐỔI SẼ CẬP NHẬT TRỰC TIẾP LÊN HỆ THỐNG THÉP TNL.</span>
                            </p>
                        </div>
                    </div>
                </div>
