...
export default function MaterialTypesPage() {
  const { data, loading, createItem, updateItem, deleteItem } = useEstimate('materialTypes');
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-sm border border-zinc-200">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-zinc-800">Vật liệu bao che</h1>
          <p className="text-sm text-zinc-500 mt-1">Quản lý danh sách các loại vật liệu bao che</p>
        </div>
            ...
            <tbody className="bg-white divide-y divide-zinc-200">
              ...
              {isAdding && (
                <tr className="bg-blue-50">
                  <td className="px-2 py-2"><input type="text" placeholder="Tên vật liệu..." value={addForm.name} onChange={e => setAddForm({...addForm, name: e.target.value})} className="w-full border border-zinc-300 p-2 text-sm rounded focus:outline-none focus:border-[#0e2188]" autoFocus /></td>
