...
// 🟢 CÔNG CỤ THÔNG DỊCH: Biến "Tên Danh Mục" thành "ten-danh-muc" để so sánh với URL
const generateSlug = (str) => {
    if (!str) return '';
    return str.toLowerCase()
        .normalize("NFD").replace(/[\u0300-\u036f]/g, "") // Xóa dấu tiếng Việt
        .replace(/đ/g, "d").replace(/Đ/g, "d")           // Chuyển chữ đ
        .replace(/[^a-z0-9]/g, "-")                      // Ký tự đặc biệt thành dấu gạch ngang
        .replace(/-+/g, "-").replace(/^-|-$/g, "");      // Dọn dẹp dấu gạch ngang thừa
};
    ...
    useEffect(() => {
        ...
        publicService.getProjects()
            .then(res => {
                ...
                const allProjects = Array.isArray(dataArray) ? dataArray : [];
                setProjects(allProjects);
                
                // 🟢 XỬ LÝ LỌC DATA TỰ ĐỘNG
                if (categoryQuery) {
                    const filtered = allProjects.filter(item => {
                        // Lấy slug từ BE (nếu có), không có thì tự dịch từ category_name
            ...
            <Container>
                ...
                {displayProjects.length > 0 ? (
                                    ...
                                    <div className="py-8 space-y-4 flex flex-col flex-grow">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-[10px] font-bold text-[#e33127] uppercase tracking-[0.2em]">
                                                {item.category_name || item.category?.name || 'Steel Construction'}
                                            </span>
                                        </div>
                    ...
                    <div className="py-32 text-center border border-dashed border-zinc-100 rounded-sm">
                        <p className="font-bold text-zinc-300 uppercase tracking-widest text-sm italic mb-4">
                            Chưa có hồ sơ dự án nào phù hợp với danh mục này
                        </p>
                        <Link href="/projects" className="text-[#e33127] font-bold uppercase text-xs tracking-widest border-b border-[#e33127] hover:text-[#0e2188] hover:border-[#0e2188] transition-colors">
                            XEM TOÀN BỘ DỰ ÁN
                        </Link>
                    </div>
                )}
...
export default function ProjectPage() {
    return (
        <main className="bg-white min-h-screen font-sans">
            <ProjectBanner />
            <Suspense fallback={
                <div className="py-40 text-center font-bold tracking-widest uppercase text-[#0e2188] animate-pulse">
                    ĐANG KẾT NỐI TỚI KHO DỮ LIỆU...
                </div>
            }>
                <ProjectContent />
            </Suspense>
