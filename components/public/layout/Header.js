      ...
      <motion.header
        ...
        className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 font-sans
          ${isInBanner
            ...
            : 'bg-white text-[#0e2188] py-4 shadow-sm border-b-[4px] border-black'} 
        `}
      >
        <Container>
          <div className="flex justify-between items-center h-full px-4 md:px-0">
            ...
            <Link href="/" className="flex items-center gap-4 group">
              ...
              <div className="flex flex-col">
                <span className={`text-xl md:text-2xl font-black uppercase tracking-tighter transition-colors leading-none
              ...
              {rootMenus.map((menu) => {
                ...
                return (
                  <div key={menu.id} className="relative group py-2">
                    <Link
                      href={menu.link || '#'}
                      className={`flex items-center gap-1 text-[11px] font-black uppercase tracking-[0.2em] relative transition-colors py-2
                    ...
                    >
                      ...
                      {hasChildren && (
                        <svg className={`w-3 h-3 transition-transform duration-300 group-hover:-rotate-180 ${isInBanner ? 'text-white' : 'text-[#0e2188]'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M19 9l-7 7-7-7" />
                        </svg>
                      )}

                      <span className={`absolute bottom-0 left-1/2 -translate-x-1/2 w-0 h-[3px] transition-all duration-300 group-hover:w-full 
                        ${isInBanner ? 'bg-white' : 'bg-[#e33127]'}`}></span>
                    </Link>

                    {/* DROPDOWN MENU CON */}
                    {hasChildren && (
                      /* Đổi mt-2 thành pt-4 để tạo thành một "vùng cầu nối vô hình" giúp giữ nguyên trạng thái hover khi di chuyển chuột xuống menu con */
                      <div className="absolute top-[100%] left-0 pt-4 w-64 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300">
                        <div className="bg-white border-[4px] border-black shadow-[6px_6px_0_0_#e33127] flex flex-col">
                          {children.map(child => (
                            <Link 
                              key={child.id} 
                              href={child.link || '#'} 
                              className="block px-6 py-4 text-xs font-black uppercase tracking-widest text-[#0e2188] hover:bg-[#e33127] hover:text-white border-b-4 border-black last:border-0 transition-colors"
                            >
                              {child.name}
                            </Link>
                          ))}
            ...
            <div className="flex items-center gap-4 xl:gap-6">
              {!loading && (
                <a
                  ...
                  className={`hidden md:flex items-center gap-3 px-6 py-3 border-[4px] border-black transition-all duration-300 text-xs font-black tracking-widest uppercase shadow-[6px_6px_0_0_#000] hover:shadow-none hover:translate-x-1 hover:translate-y-1
                    ${isInBanner
                      ? 'bg-white text-black hover:bg-[#e33127] hover:text-white hover:border-[#e33127]'
                      : 'bg-[#e33127] text-white hover:bg-black'}`}
                >
              ...
              <button
                ...
                className={`lg:hidden p-2 transition-all border-[4px] shadow-[4px_4px_0_0_#000] active:shadow-none active:translate-x-1 active:translate-y-1
                  ${isInBanner 
                    ? 'border-white text-white hover:bg-white hover:text-black' 
                    : 'border-black text-black hover:bg-[#e33127] hover:text-white hover:border-[#e33127]'}`}
              >
                <MdMenu size={24} />
              </button>
            </div>
